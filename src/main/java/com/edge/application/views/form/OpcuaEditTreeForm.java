...
@PageTitle("Opcua Edit Tree Form")
@Route(value = "opcuaedittreeform" ,layout = MainLayout.class)
public class OpcuaEditTreeForm extends VerticalLayout implements HasUrlParameter<String>{
	
	public static final String ROUTE_NAME="opcuaedittreeform";
	...
	public void init(String para)
	{
		...
		btn_tag.addClickListener(e -> {
			...
			String url = service.getEndpointurl(global_var,service.get_Source_name(global_var));

			UI.getCurrent().getPage().executeJavaScript("window.open('OpcuaEditTree.html?url="+url+"','popUpWindow','height=300,width=700,left=50,top=50,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no, status=yes');");
		});
