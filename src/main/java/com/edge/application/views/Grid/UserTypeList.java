...
@Route(value = "usertypelist",layout = MainLayout.class)
public class UserTypeList extends VerticalLayout{
	
	public static final String ROUTE_NAME="usertypelist";
	Button btn_add = new Button("User Permission Form");
	
	TextField txt_search = new TextField();
	Label heading= new Label();
	
	Grid<UserDetailsTable> grid = new Grid<UserDetailsTable>();
	ListDataProvider<UserDetailsTable> dataProvider;
	...
	public void init()
	{
		...
		grid.setWidthFull();
		heading.setText("User Type List");
		...
		btn_add.addClickListener(e->{
			UI.getCurrent().navigate(UserDetailsForm.ROUTE_NAME+"/0");
		});
		update();
		...
		Grid.Column<UserDetailsTable> Raw_Material = grid.addColumn(UserDetailsTable::getUser_id).setHeader("User ID")
				.setAutoWidth(true).setResizable(true).setSortable(true).setAutoWidth(true)
				.setTextAlign(ColumnTextAlign.CENTER);
		
		Grid.Column<UserDetailsTable> sap_id = grid.addColumn(UserDetailsTable::getuser_name).setHeader("User Name")
				.setAutoWidth(true).setResizable(true).setSortable(true).setAutoWidth(true)
				.setTextAlign(ColumnTextAlign.CENTER);
		
		Grid.Column<UserDetailsTable>elem_id = grid.addColumn(UserDetailsTable::getType).setHeader("Type")
				.setAutoWidth(true).setResizable(true).setSortable(true).setAutoWidth(true)
				.setTextAlign(ColumnTextAlign.CENTER);
		...
		Grid.Column<UserDetailsTable> editsource = grid.addComponentColumn(editdata -> {
			...
			addinst.addClickListener(ed -> {
				UI.getCurrent().navigate(UserDetailsForm.ROUTE_NAME + "/" + editdata.getId());
			});
			return addinst;
		...
		/*
				...
				Grid.Column<UserDetailsTable> deleteColumn = grid.addComponentColumn(sourcedata -> {
					...
					delete.addClickListener(ed -> {
							...
							dialog.add(new Text("Are you sure you want to delete user permission...?"));
	...
	public void update() {
		...
		List<UserDetailsTable> list = service.user_permission_list();
